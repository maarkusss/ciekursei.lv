                            ...
                            <div class="flex flex-col pb-6 border-b">
                                <div class="grid grid-cols-1 gap-4 text-justify lg:grid-cols-2">
                                    <a href="{{ route('section.deep', ['name' => $section->route_name, 'deep_name' => $blogPost->route_name]) }}"
                                        ...
                                        <div class="flex items-center justify-center">
                                            <img class="object-contain max-w-full max-h-96 rounded-xl"
                                                src="{{ $blogPost->image }}" alt="{{ $blogPost->image_alt_text }}"
                                                loading="lazy" title="{{ $blogPost->image_alt_text }}">
                                        </div>
                                    </a>
                                    <div class="flex flex-col items-center justify-between">
                                        ...
                                        <span
                                            class="pb-4 text-sm text-right text-gray-500 group-hover:text-primary-dark group-hover:underline">
                                            {{ date('Y-m-d', strtotime($blogPost->created_at)) }}
                                        </span>
                                        <div
                                            class="max-w-full mb-4 prose text-left text-md lg:line-clamp-4 line-clamp-2 md:mb-0">
                                            {!! $blogPost->source !!}
                                        </div>
